...
// Task 2 (DONE): Import the `useDispatch()` method from the appropriate package
// Task 3 (DONE): Import the `addPhoto()` action creator from the photos slice

import './create.css';

export default function CreatePhoto() {
  const [formData, setFormData] = useState({ imageUrl: '', caption: '' });
  // Task 4 (DONE): Store a reference to the Redux store's dispatch method in a variable called `dispatch`
  ...
  function handleSubmit(event) {
    ...
    // Task 5 (DONE): Dispatch the `addPhoto()` action creator, passing in the form data
